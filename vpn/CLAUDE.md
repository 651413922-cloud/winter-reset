...
- **Dynamic generation, never reuse/patch a stale config** — `adapters/xray_config.py` recreates the Xray config from the node data (mirroring v2rayN's `GenerateClientConfigContent`), rather than editing an existing `config.json`. This avoids the "wrong DNS / dead node" class of bugs.
