...
class RuntimeManager:
    ...
    def __init__(self, exe_path: str = None,
        ...
        # timeout=10（config.TIMEOUT 默认 20）：守护/恢复路径的健康检查
        # 若用默认 20s，死节点每次检查耗时 25s，恢复节奏完全钝化。
    ...
    def start(self) -> bool:
        """
        Start proxy process(es) based on v2rayN's configured core type.

        Xray (PAC/system-proxy): start xray.exe only — no admin needed.
        sing-box (TUN): start sing-box + Xray dual-process — admin required.
        """
        core_type = get_active_core_type()

        if core_type == CORE_TYPE_XRAY:
            return self._start_xray_mode()
        else:
            return self._start_singbox_mode()

    def _start_xray_mode(self) -> bool:
        """Xray-only start (PAC / system-proxy mode). No admin required."""
        if self.is_running():
        ...
        logger.info("Xray running — PAC proxy ready")
        return True

    def _start_singbox_mode(self) -> bool:
        """
        Dual-process start: sing-box (TUN) then Xray (proxy).

        Requires admin rights for TUN virtual interface.
        """
    ...
    def _start_xray(self) -> bool:
        ...
        try:
            ...
            env = os.environ.copy()
            # Xray needs geosite.dat/geoip.dat in its exe directory,
            # but v2rayN stores them one level up in bin/.
            asset_dir = os.path.dirname(os.path.dirname(self.xray_exe))
            ...
            while time.time() < deadline:
                if self.is_xray_running():
                    ...
                    # Wait until the SOCKS inbound is actually listening so
                    # callers that probe the proxy immediately don't hit a
                    # 'connection refused / port not listening' race.
                    self.wait_for_proxy_ready(timeout=4.0)
                    return True
                time.sleep(0.3)
            ...
            if poll is not None:
                ...
                err_msg = stderr.decode('utf-8', errors='replace')[:1200] if stderr else ''
                logger.error("Xray exited immediately (code: %d)", poll)
                for line in err_msg.split('\n'):
                    stripped = line.strip()
                    if stripped:
                        logger.error("  %s", stripped)
            else:
    ...
    def restart_xray(self) -> bool:
        ...
        if not self._start_xray():
            return False
        # _start_xray already waits for the port; this is a belt-and-braces
        # guarantee that the SOCKS inbound is up before callers probe it.
        self.wait_for_proxy_ready(timeout=4.0)
        return True

    def wait_for_proxy_ready(self, timeout: float = 6.0) -> bool:
        """Block until the SOCKS inbound port is accepting connections.

        Returns False on timeout (process may have crashed or bound the
        wrong port).
        """
        import socket
        try:
