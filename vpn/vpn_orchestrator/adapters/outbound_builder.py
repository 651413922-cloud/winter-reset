...
def _build_shadowsocks_outbound(profile: ProfileItem) -> dict:
    """ConfigType=3/4: Shadowsocks in Xray format.

    The SS method is stored in ProtoExtra.SsMethod (v2rayN schema), NOT in
    the Security column — so we read it from there, falling back to the
    Security column, then to a sane default.
    """
    ss_method = (
        profile.proto_extra_dict.get('SsMethod')
        or profile.security
        or 'aes-256-gcm'
    )
    ob = {
